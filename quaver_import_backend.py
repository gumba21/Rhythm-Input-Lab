...
_TEMP_ROOT = Path(tempfile.gettempdir()) / "RhythmInputLab" / "quaver-imports"
...
def _preview_source(path: Path) -> dict[str, Any]:
    preview = quaver_importer.inspect_source(path)
    ...
    preview["capabilities"] = {
        "preserved": ["notes", "holds", "mines", "BPM", "time signatures", "SV", "metadata", "timing groups", "bookmarks"],
        "extension_stored": ["keysounds", "custom samples", "editor layers", "scroll-speed-factor keyframes"],
        "approximated": ["mines use RIL hazard behavior"],
        "not_rendered": ["Quaver skins", "per-group scroll presentation", "keysound playback"],
    }
    return preview
...
def _upload_chunk(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    filename = Path(str(payload.get("filename") or "mapset.qp")).name
    suffix = Path(filename).suffix.casefold()
    if suffix not in {".qua", ".qp"}:
        raise ValueError("Choose a standalone .qua chart or a .qp Quaver mapset")
    index = int(payload.get("index"))
    total = int(payload.get("total"))
    if total < 1 or total > 50_000 or index < 0 or index >= total:
        raise ValueError("Invalid Quaver upload chunk")
    raw = base64.b64decode(str(payload.get("data") or ""), validate=True)
    if len(raw) > _MAX_CHUNK_BYTES:
        raise ValueError("Quaver upload chunk is too large")
    ...
    with _LOCK:
        ...
        if sum(part.stat().st_size for part in parts) > _MAX_UPLOAD_BYTES:
            ...
            raise ValueError("The Quaver source exceeds the 2 GB safety limit")
        result: dict[str, Any] = {"upload_id": upload_id, "received": len(parts), "total": total, "complete": False}
        if len(parts) != total:
            return result
        ...
        with source_path.open("wb") as output:
            for part_index in range(total):
                ...
                if not part.exists():
                    raise ValueError("Quaver upload is missing a chunk")
...
def _unique_folder(root: Path, song_name: str) -> Path:
    base = core.safe_name(song_name, "Imported Quaver song")
    candidate = root / base
    suffix = 2
...
def _write_song(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="ril_quaver_chart_") as temp_dir:
        original = Path(temp_dir) / core.safe_name(source_name, "original.qua")
    ...
    audio_saved = False
    if audio_source and audio_source.is_file() and audio_source.suffix.casefold() in quaver_importer.SUPPORTED_AUDIO_SUFFIXES:
    ...
    quaver_meta = bundle.get("song_metadata", {}).get("quaver", {})
    song_meta.update({
        "song_name": song_name,
        ...
        "imported_chart": {
            ...
            "mines": int(summary.get("mine_notes") or 0),
            "events": int(summary.get("event_count") or 0),
            "importer_version": quaver_importer.IMPORTER_VERSION,
        ...
        "provenance": {
            ...
            "map_set_id": summary.get("map_set_id"),
        },
        "quaver": {
            "qua_version": summary.get("qua_version"),
            "map_id": summary.get("map_id"),
            "map_set_id": summary.get("map_set_id"),
            "mode": summary.get("mode"),
            "has_scratch_key": summary.get("has_scratch_key"),
            "audio_filename": summary.get("audio_filename"),
            "source_sha256": summary.get("source_sha256"),
            "source": quaver_meta.get("source", ""),
            "tags": quaver_meta.get("tags", ""),
            "genre": quaver_meta.get("genre", ""),
        },
    })
...
def _selected_ids(payload: dict[str, Any], preview: dict[str, Any]) -> set[str]:
    ...
    if not selected:
        raise ValueError("Select at least one supported Quaver difficulty")
    return selected
...
def _commit_import(app: Any, upload_id: str, payload: dict[str, Any], app_version: str) -> dict[str, Any]:
    ...
    if len(source_paths) != 1:
        raise FileNotFoundError("Uploaded Quaver source was not found")
    ...
    if source_path.suffix.casefold() == ".qua":
        ...
        if str(diff["id"]) in selected:
            imported.append(_write_song(
                ...
                mode=mode, package_name=package_name, audio_source=None,
                audio_original_name=str(bundle["summary"].get("audio_filename") or ""),
                linked_audio=linked_audio, app_version=app_version,
            ))
    else:
        names = quaver_importer.validate_qp(source_path)
        ...
        with zipfile.ZipFile(source_path, "r") as archive:
            for entry_name, diff in diff_by_entry.items():
                ...
                if not info:
                    raise ValueError(f"Selected chart disappeared from package: {entry_name}")
                source_bytes = archive.read(info)
                bundle = quaver_importer.parse_qua_bytes(source_bytes, Path(entry_name).name)
                ...
                if audio_entry_name:
                    ...
                    if not audio_path:
                        ...
                        if audio_info and Path(audio_info.filename).suffix.casefold() in quaver_importer.SUPPORTED_AUDIO_SUFFIXES:
                ...
                imported.append(_write_song(
                    ...
                    mode=mode, package_name=package_name, audio_source=audio_path,
                    audio_original_name=str(bundle["summary"].get("audio_filename") or ""),
                    linked_audio=linked_audio, app_version=app_version,
                ))
    if not imported:
...
def install_quaver_import_endpoint(backend: ModuleType) -> None:
    """Install chunked standalone .qua and multi-difficulty .qp importing."""
    handler = backend.Handler
    if getattr(handler, "_ril_quaver_import_installed", False):
        return
    _cleanup_temp()
    ...
    def do_POST(self) -> None:  # type: ignore[no-untyped-def]
        ...
        if path not in {"/api/quaver/import/chunk", "/api/quaver/import/commit", "/api/quaver/import/cancel"}:
            return original_post(self)
        try:
            payload = self._body_json()
            if path == "/api/quaver/import/chunk":
                ...
                if not _UPLOAD_ID.match(upload_id):
                    raise ValueError("Invalid Quaver upload id")
                upload_dir = _TEMP_ROOT / upload_id
                if path == "/api/quaver/import/cancel":
                    shutil.rmtree(upload_dir, ignore_errors=True)
    ...
    handler._ril_quaver_import_installed = True
